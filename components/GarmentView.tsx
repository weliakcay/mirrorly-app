      ...
      <div className="min-h-full flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center px-6 pt-6 pb-8">
          <div className="w-full max-w-sm space-y-4">
            ...
            <button
              onClick={onContinue}
              className="group w-full relative bg-mirrorly-black text-white py-5 rounded-xl shadow-xl hover:bg-mirrorly-stone hover:scale-[1.02] transition-all duration-300 active:scale-95"
            >
            ...
            {/* Boutique Info Card */}
            <div className="rounded-2xl border border-mirrorly-paper bg-white/92 px-4 py-5 shadow-sm">
              ...
              {merchantProfile.logoUrl ? (
                ...
                <h2 className="font-serif text-xl text-mirrorly-black mb-2">{merchantProfile.name}</h2>
              )}
              {merchantProfile.description && (
                <p className="text-xs text-mirrorly-stone leading-relaxed mb-3">
                  {merchantProfile.description}
                </p>
              )}
            </div>
            ...
            <button
              ...
              className="w-full rounded-2xl border border-mirrorly-paper bg-white/92 px-4 py-4 text-left shadow-sm hover:shadow-md transition-all"
            >
            ...
            {otherItems.length > 0 && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3 text-mirrorly-stone hover:text-mirrorly-black transition-colors"
              >
            ...
            <div className="grid grid-cols-2 gap-4">
              {otherItems.map((item) => (
                <button
                  ...
                  className="text-left bg-mirrorly-cream rounded-xl overflow-hidden hover:shadow-lg transition-shadow"
                >
                  <img src={item.imageUrl} alt={item.name} className="w-full aspect-[3/4] object-cover" />
                  <div className="p-3">
                    <h4 className="font-serif text-sm text-mirrorly-black truncate">{item.name}</h4>
                    <p className="text-xs text-mirrorly-stone font-medium">{formatPrice(item.price, merchantProfile.currency || item.currency)}</p>
                  </div>
                </button>
              ))}
            </div>
