      ...
      <div className="flex-1 flex flex-col justify-center px-6 pt-20 pb-10">
        <div className="w-full max-w-md mx-auto space-y-8">
          <div className="text-center space-y-2">
            <p className="text-mirrorly-gold text-[11px] uppercase tracking-[0.3em] font-semibold">{t('Adim 02 · Kendini Cek')}</p>
            <h2 className="font-serif text-4xl text-mirrorly-black">{t('Goruntunu yukle')}</h2>
            <p className="font-sans text-mirrorly-stone font-light leading-relaxed">
              {t('Net kadraj ve iyi isik, daha iyi bir deneme sonucu verir.')}
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 rounded-2xl border border-mirrorly-gold/25 bg-mirrorly-paper/40 px-4 py-3">
              <img
                ...
                alt={t('Ideal fotograf ornegi')}
                loading="lazy"
                onError={(e) => { (e.currentTarget as HTMLImageElement).style.visibility = 'hidden'; }}
                className="w-16 h-16 rounded-xl object-cover border border-white shadow-sm shrink-0"
              />
              <div className="text-left">
                <p className="text-[10px] uppercase tracking-[0.22em] text-mirrorly-gold font-semibold mb-0.5">{t('Ornek cekim')}</p>
                <p className="text-xs text-mirrorly-stone leading-snug">{t('Boyle net kadrajli, iyi isikli bir fotograf ideal sonucu verir.')}</p>
              </div>
            </div>
            ...
            >
              ...
              <span className="font-serif text-xl text-mirrorly-black">{t('Fotograf cek')}</span>
            </button>
          ...
          <div className="rounded-[1.75rem] bg-mirrorly-paper/25 border border-mirrorly-paper/70 px-5 py-4 text-sm text-mirrorly-stone leading-relaxed">
            {t('Tek bir fotograf yeterli. Urunun gorunecegi bolgeyi kapatmayan, temiz ve net bir kare secmeye calis.')}
          </div>

          {/* Rehber ipuçları */}
          <div className="mt-4 p-4 bg-mirrorly-paper/40 rounded-2xl border border-mirrorly-paper space-y-2">
            <p className="text-xs font-medium text-mirrorly-gold uppercase tracking-[0.2em]">{t('En iyi sonuç için')}</p>
