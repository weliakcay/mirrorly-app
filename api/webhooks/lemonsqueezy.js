...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!merchantUid) {
      console.warn("Webhook ignoring event without merchant_uid");
      return res.status(200).json({ message: 'Ignored: No merchant_uid found' });
    }

    if (eventName === 'order_created' || eventName === 'subscription_created' || eventName === 'subscription_updated') {
      const { addCreditsToMerchant } = await import("../../server/billingService.js");
      
      const variantId = String(data.attributes.variant_id);
      let creditsToAdd = 0;
      let newTier = undefined;

      // Mock package definitions mapped to LemonSqueezy Variant IDs
      ...
      if (creditsToAdd > 0) {
        ...
        console.log(`Granted ${creditsToAdd} credits to ${merchantUid}`);
      }
    }
